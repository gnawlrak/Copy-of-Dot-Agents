...
const GameCanvas = ({ level, loadout, operator, onMissionEnd, showSoundWaves, agentSkinColor, customControls, aimSensitivity, onAimSensitivityChange, onCustomControlsChange, defaultControlsLayout, difficulty, isMultiplayer, networkClient, onScoreChange, initialRunScore, totalScore, highScore }: GameCanvasProps): JSX.Element => {
  ...
  const [roomScores, setRoomScores] = useState<Record<string, { kills: number; deaths: number; name?: string; team?: 'red' | 'blue' }>>({});
  const roomScoresRef = useRef<Record<string, { kills: number; deaths: number; name?: string; team?: 'red' | 'blue' }>>({});
  const [totalRoomKills, setTotalRoomKills] = useState(0);
  const [teamScores, setTeamScores] = useState<{ red: number; blue: number }>({ red: 0, blue: 0 });
  const teamScoresRef = useRef<{ red: number; blue: number }>({ red: 0, blue: 0 });
  ...
  const [matchEnded, setMatchEnded] = useState<{ mode: 'tdm' | 'ffa' | '1v1'; winner: 'red' | 'blue' | 'draw' | null; teamScores?: { red: number; blue: number }; players: Array<{ id: string; name: string; team?: string; kills: number; deaths: number }> } | null>(null);
    ...
    const handleRoomUpdate = (room: any) => {
      ...
      if (room && room.players) {
        ...
        if (localPlayer && localPlayer.team) {
          ...
          setShowTeamSelect(prev => prev ? false : prev);
        }
      }
    };
  ...
  const syncPlayerState = useCallback(() => {
    ...
    const playerState: PlayerState = {
      ...
      name: networkClient?.ownName || 'Anonymous',
      x: player.x,
      y: player.y,
  ...
  const drawRemotePlayers = useCallback((ctx: CanvasRenderingContext2D, scale: number, cameraScale: number) => {
    ...
    players.forEach(player => {
      ...
      ctx.fillText(player.name || player.playerId || player.id || 'UNKNOWN', x, y + 22 * scale);
  ...
  const drawTeamInfo = useCallback((ctx: CanvasRenderingContext2D, scale: number, cameraScale: number) => {
    // Intentionally left empty; team/room info is shown in React HUD overlays instead.
  }, [isMultiplayer]);
  ...
  const startDoorInteraction = useCallback(() => {
      ...
      // Calculate door direction vector and normal vector pointing to the swing-open side
      const doorVecX = Math.cos(doorToInteract.closedAngle);
      const doorVecY = Math.sin(doorToInteract.closedAngle);
      
      // The side the door swings into
      const normalX = -doorVecY * doorToInteract.swingDirection;
      const normalY = doorVecX * doorToInteract.swingDirection;
  ...
  const meleeAttack = () => {
    ...
    if (currentMelee.name === 'Riot Shield') {
        ...
        // Shield bash damage / stun against remote players in multiplayer
        if (isMultiplayer && networkClient) {
            for (const rp of remotePlayersRef.current) {
                if (rp.health <= 0) continue;
                const dx = rp.x - player.x;
                const dy = rp.y - player.y;
                const dist = Math.hypot(dx, dy);
                const rpRadius = rp.radius || 10;
                if (dist < bashRange + rpRadius) {
                    const angleToEnemy = Math.atan2(dy, dx);
                    const angleDiff = Math.abs(normalizeAngle(angleToEnemy - playerDirection));
        ...
        if (isMultiplayer) {
            if (!(player as any).isRespawning) {
                ...
                if (killerId && killerId !== 'unknown') {
                    ...
                    const killerName = remotePlayersRef.current.find(p => p.id === killerId)?.name || killerId;
                    setPurchaseMessage(`ELIMINATED BY ${killerName}`);
                } else {
  ...
  const throwThrowable = (customLaunchPower?: number) => {
    ...
    throwablesRef.current.push({
        ...
        radius: 5 * scale,
        hasBounced: false,
        ownerId: isMultiplayer ? networkClient?.ownId : undefined,
    });
};
  ...
  useEffect(() => {
    ...
    const setupMap = () => {
        ...
        level.doors.forEach(d => {
            // Precise orientation-locked door scale:
            // Horizontal doors span X gaps of walls (scaled by canvas.width),
            // vertical doors span Y gaps of walls (scaled by canvas.height).
            // This maintains perfect gap closure and scale alignment across ALL resolutions & aspect ratios.
            const cosAngle = Math.abs(Math.cos(d.closedAngle));
            const sinAngle = Math.abs(Math.sin(d.closedAngle));
            
            let lengthPixels = 0;
            if (cosAngle > sinAngle) {
                lengthPixels = d.length * canvas.width;
            } else {
                lengthPixels = d.length * canvas.height;
            }
    ...
    const initializePlayer = () => {
      ...
      if (isMultiplayer) {
          ...
          let attempts = 0;
          const radius = 10 * scale / cameraScale;
          while (attempts < 500) {
              ...
              for (const wall of level.walls) {
                  const wx = wall.x * canvas.width;
                  const wy = wall.y * canvas.height;
                  const ww = wall.width * canvas.width;
                  const wh = wall.height * canvas.height;
      ...
      const createWeaponInstance = (weaponName: string, attachments: { [slot: string]: string }): Weapon => {
          ...
          } else if (def.name === 'Riot Shield') {
              // Ensure shield always has durability even if base definition lacks it
    ...
    const gameLoop = () => {
      ...
      const isRespawning = isMultiplayer && (player as any).isRespawning;
      if (isRespawning) {
          isShootingRef.current = false;
          (player as any).respawnTimer = Math.max(0, ((player as any).respawnTimer || 0) - dt);
          if ((player as any).respawnTimer <= 0) {
              (player as any).isRespawning = false;
              (player as any).respawnTimer = 0;
              
              // Find random safe coordinates on spawn
              let spawnX = level.playerStart.x * canvas.width;
              let spawnY = level.playerStart.y * canvas.height;
              let attempts = 0;
              const radius = 10 * scale / (cameraScaleRef.current || 1.0);
              while (attempts < 500) {
                  const rx = 0.1 + Math.random() * 0.8;
                  const ry = 0.1 + Math.random() * 0.8;
                  const px = rx * canvas.width;
              ...
              // Reset shield durability to full on respawn
              ...
              setPurchaseMessage('RESPAWNED - ENGAGING HOSTILES');
      ...
      const detonate = (n: Throwable) => {
        ...
        // Apply throwable effects to remote players in multiplayer
        if (isMultiplayer && networkClient) {
            for (const rp of remotePlayersRef.current) {
                if (rp.health <= 0) continue;
                const dist = Math.hypot(rp.x - n.x, rp.y - n.y);
                if (dist > rad) continue;

                let isObstructed = false;
                for (const segment of dynamicSegments) {
                    if (intersectSegSeg(n.x, n.y, rp.x, rp.y, segment)) {
                        isObstructed = true;
                        break;
                    }
                }
                ...
                if (n.type === 'grenade') {
                    ...
                    const damage = (1 - (dist / rad)) * 100;
                    (networkClient as any).send('player-hit', {
                        targetId: rp.id,
                        damage,
                        attackerId: n.ownerId || networkClient.ownId,
                        impact: { x: rp.x, y: rp.y },
                        sourceDir: { x: rp.x - n.x, y: rp.y - n.y }
                    });
                } else if (n.type === 'flashbang') {
                    if (isObstructed) continue;
                    // Flash effect is applied locally on the remote player's client via player-action
                    sendPlayerAction('flashbang', { targetId: rp.id, x: n.x, y: n.y });
                }
            }
        }
    };
    ...
    const createExplosion = (x: number, y: number, radius: number, maxDamage: number, dynamicSegments: Segment[]) => {
      ...
      const isTryingToFire = (touchState.fire.id !== null || touchState.fixedFire.id !== null || isShootingRef.current) && !isAimingThrowableRef.current && !(isMultiplayer && (player as any).isRespawning);
...
doorsRef.current.forEach(door => {
    ...
    if (isBlocked) {
      ...
      if (!isEnded && !(isMultiplayer && (player as any).isRespawning)) {
        let dx = 0; let dy = 0;
      ...
      if (slash.active) {
        ...
        // Knife slash against remote players in multiplayer
        if (isMultiplayer && networkClient) {
            for (const rp of remotePlayersRef.current) {
                if (rp.health <= 0) continue;
                ...
                (networkClient as any).send('player-hit', {
                    targetId: rp.id,
                    damage: 100, // Knife slash is instant kill
                    attackerId: networkClient.ownId,
                    impact: { x: rp.x, y: rp.y },
                    sourceDir: { x: Math.cos(slash.curA), y: Math.sin(slash.curA) }
                });
            }
        }
      ...
      if (isMultiplayer) {
        ...
        for (const patch of firePatchesRef.current) {
            ...
            if (isMultiplayer && networkClient) {
                for (const rp of remotePlayersRef.current) {
                    if (rp.health <= 0) continue;
                    if (Math.hypot(rp.x - patch.x, rp.y - patch.y) < (rp.radius || 10) + patch.radius) {
                        (networkClient as any).send('player-hit', {
                            targetId: rp.id,
                            damage: patch.damagePerSecond * 0.5, // tick damage scaled for server
                            attackerId: networkClient.ownId,
                            impact: { x: rp.x, y: rp.y },
                            sourceDir: { x: 0, y: 0 }
                        });
                                ...
                                if (enemy.type === 'advanced') {
                                    ...
                                    if (enemy.axeState === 'swing') {
                                            ...
                                            if (shield && shield.durability && isEquipped && playerAngleDiff < Math.PI / 2) {
                                                ...
                                                applyDamageToPlayer(1000, {x: Math.cos(enemy.direction), y: Math.sin(enemy.direction)}, {x: player.x, y: player.y}, 'enemy_axe');
                                            }
                                            }
      ...
      if (isExtractionActiveRef.current && !isMissionCompleteRef.current) {
        ...
        context.shadowColor = 'black';
        context.shadowBlur = 10 * scale;
        context.fillText(t('extraction').toUpperCase(), zone.x + zone.width / 2, zone.y + zone.height / 2 + 8 * scale);
        context.shadowBlur = 0;
      }
      ...
      if (enemyRangeDisplayTimerRef.current > 0) {
          ...
          context.textAlign = 'center';
          context.fillText(`${t('enemyDensity')}: ${rangeInfo}`, canvas.width / 2, 40 * scale);
          context.restore();
      }
      ...
      if (!isEnded) {
          // Centered Premium Dashboard Dock
          const dockW = Math.min(canvas.width - 40 * scale, 480 * scale);
          const dockH = 68 * scale;
          const dockX = (canvas.width - dockW) / 2;
          const dockY = canvas.height - dockH - (12 * scale); // Floats above the edge
          ...
          // Glassmorphic body
          context.fillStyle = 'rgba(15, 23, 42, 0.85)'; // slate-900 border
          context.strokeStyle = 'rgba(75, 85, 99, 0.4)'; 
          context.lineWidth = 1.5 * scale;
          
          context.beginPath();
          if (typeof context.roundRect === 'function') {
              context.roundRect(dockX, dockY, dockW, dockH, 8 * scale);
          } else {
              context.rect(dockX, dockY, dockW, dockH);
          }
          context.fill();
          context.stroke();
          ...
          context.strokeStyle = 'rgba(75, 85, 99, 0.3)';
          context.beginPath();
          context.moveTo(dockX + dockW / 2, dockY + 12 * scale);
          context.lineTo(dockX + dockW / 2, dockY + dockH - 12 * scale);
          context.stroke();

          // --- LEFT DOCK AREA: Health / Shield & Heals ---
          const leftX = dockX + 16 * scale;
          ...
          if (healthPercentage < 0.6) healthColor = '#f59e0b'; // Amber 500
          if (healthPercentage < 0.3) healthColor = '#ef4444'; // Red 500
          
          let drawY = dockY + 16 * scale;
          // Background
          context.fillStyle = 'rgba(16, 185, 129, 0.15)';
          context.fillRect(leftX, drawY, barW, barH);
          // Foreground
          context.fillStyle = healthColor;
          context.fillRect(leftX, drawY, barW * healthPercentage, barH);
          // Text
          context.font = `bold ${9 * scale}px mono`;
          context.fillStyle = '#cbd5e1';
          context.textAlign = 'left';
          context.textBaseline = 'middle';
          context.fillText(`${t('health')}: ${Math.round(player.health)}/${player.maxHealth}`, leftX, drawY - 6 * scale);

          // Shield block or Armor block
          drawY = dockY + 38 * scale;
          const hasShield = shield && shield.maxDurability;
          if (hasShield) {
              const shieldPercentage = Math.max(0, shield.durability || 0) / shield.maxDurability;
              context.fillStyle = 'rgba(96, 165, 250, 0.15)';
              context.fillRect(leftX, drawY, barW, barH);
              context.fillStyle = '#3b82f6'; // Blue 500
              context.fillRect(leftX, drawY, barW * shieldPercentage, barH);
              
              context.font = `bold ${9 * scale}px mono`;
              context.fillStyle = '#cbd5e1';
              context.fillText(`${t('shield')}: ${Math.round(shield.durability)}/${shield.maxDurability}`, leftX, drawY - 6 * scale);
          } else {
              context.fillStyle = 'rgba(148, 163, 184, 0.05)';
              context.fillRect(leftX, drawY, barW, barH);
              
              context.font = `bold ${9 * scale}px mono`;
              context.fillStyle = '#64748b'; // Slate 500
              context.fillText(t('armor').toUpperCase(), leftX, drawY - 6 * scale);
          }

          // Medkits counter
          const medX = leftX + barW + 18 * scale;
          const medY = dockY + dockH / 2;
          context.font = `bold ${18 * scale}px mono`;
          context.fillStyle = '#10b981';
          context.textAlign = 'left';
          context.textBaseline = 'middle';
          context.fillText('+', medX, medY + (1 * scale));
          
          context.font = `bold ${14 * scale}px mono`;
          context.fillStyle = 'white';
          context.fillText(player.medkits.toString(), medX + 11 * scale, medY + (1 * scale));

          // --- RIGHT DOCK AREA: Weapon & Throwable ---
          const rightX = dockX + dockW / 2 + 16 * scale;
          
          let weaponKey = '';
          if (!hasUsedTouchRef.current) {
               if (player.currentWeaponIndex === 0) weaponKey = '[1]';
               else if (player.currentWeaponIndex === 1) weaponKey = '[2]';
               else if (player.currentWeaponIndex === 2) weaponKey = '[3]';
               else if (player.currentWeaponIndex === 3) weaponKey = '[4]';
          }
          const weaponKeyStr = weaponKey ? ` ${weaponKey}` : '';

          // Weapon details text
          const wname = currentWeapon && currentWeapon.name ? String(currentWeapon.name).toUpperCase() : 'NO WEAPON';
          const fireModeText = (currentWeapon && currentWeapon.category !== 'melee' && currentWeapon.currentFireMode) 
            ? ` ${String(currentWeapon.currentFireMode).toUpperCase()}` 
            : '';
          
          context.textBaseline = 'top';
          context.textAlign = 'left';
          context.font = `bold ${11 * scale}px mono`;
          context.fillStyle = '#5eead4'; // Teal 300
          context.fillText(wname + fireModeText + weaponKeyStr, rightX, dockY + 12 * scale);

          // Ammo status text
          let ammoText = '';
          if (player.isReloading) {
              ammoText = t('reload').toUpperCase() + '...';
              context.fillStyle = '#f59e0b';
          } else if (currentWeapon) {
              if (currentWeapon.category === 'melee') {
                  const isReady = player.shootCooldown <= 0;
                  ammoText = isReady ? t('readyBtn').toUpperCase() : `${t('reload').toUpperCase()}: ${player.shootCooldown.toFixed(1)}s`;
                  context.fillStyle = isReady ? 'white' : '#94a3b8';
              } else {
                  ...
                  context.fillStyle = (currentWeapon.magSize !== -1 && currentWeapon.ammoInMag === 0) ? '#f43f5e' : 'white';
              }
          }
          context.font = `bold ${15 * scale}px mono`;
          context.fillText(ammoText, rightX, dockY + 28 * scale);

          // Throwable representation on the rightmost edge of dock
          const throwX = dockX + dockW - 16 * scale;
          context.textAlign = 'right';
          context.textBaseline = 'top';
          if (currentThrowableType) {
              context.font = `bold ${9 * scale}px mono`;
              context.fillStyle = '#94a3b8'; // Slate 400
              context.fillText(currentThrowableType.toUpperCase(), throwX, dockY + 12 * scale);

              const countStr = level.name === 'TRAINING GROUND' ? '∞' : `${throwableCount}`;
              context.font = `bold ${15 * scale}px mono`;
              context.fillStyle = throwableCount > 0 ? '#f43f5e' : '#475569';
              context.fillText(countStr, throwX, dockY + 28 * scale);
          }

          if (!hasUsedTouchRef.current) {
              context.font = `${10 * scale}px mono`;
              context.fillStyle = '#64748b'; // Slate 500
              context.textAlign = 'center';
              context.fillText(`[G] MODE | [R] RELOAD | [Q] MELEE | [T] GRENADE | [F] THROW`, dockX + dockW / 2, dockY + dockH + 12 * scale);
          }
      ...
      if (touchState.joystick.id === null) {
          ...
          if (takedownHintEnemyRef.current) { // Priority 1: Takedown
            ...
            if(pointInPoly(enemy.x, enemy.y, viewPoly)) {
                ...
                context.fillText(t('takedown'), enemyScreenPos.x, enemyScreenPos.y - enemy.radius * 2);
                context.shadowBlur = 0;
            }
      ...
      if (isEnded) {
          ...
          if (now - (missionEndTimeRef.current ?? 0) > endDelay) {
              ...
              if (isMissionCompleteRef.current) {
                  context.fillStyle = '#10b981';
                  context.fillText(t('missionSuccess').toUpperCase(), cx, py + 52 * scale);
              } else {
                  context.fillStyle = '#ef4444';
                  context.fillText(t('died').toUpperCase(), cx, py + 52 * scale);
              }
              context.shadowBlur = 0;
              ...
              context.font = `bold ${18 * scale}px mono`;
              context.fillStyle = '#cbd5e1';
              context.fillText(`${t('missionTime')}:`, leftX, rowY);
              ...
              context.font = `bold ${18 * scale}px mono`;
              context.fillStyle = '#cbd5e1';
              context.fillText(`${t('kills')}:`, leftX, rowY);
              context.font = `${18 * scale}px mono`;
              context.fillStyle = 'white';
              ...
              context.font = `bold ${18 * scale}px mono`;
              context.fillStyle = '#cbd5e1';
              context.fillText(`${t('score')}:`, rightX - 80 * scale, rowY);
              ...
              context.fillText(`${t('projectedTotal')}:`, rightX - 80 * scale, rowY);
              ...
              context.fillText(`${t('highScore')}:`, rightX - 80 * scale, rowY);
              context.font = `${16 * scale}px mono`;
              ...
              context.font = `${18 * scale}px mono`;
              context.fillStyle = '#94a3b8';
              context.fillText(t('pressToRespawn'), cx, py + panelH - 32 * scale);
          }
      }
      
      const buttonRadius = 25 * scale;
      pauseButtonCircleRef.current = { x: canvas.width - buttonRadius - 20 * scale, y: buttonRadius + 20 * scale, r: buttonRadius };
      ...
      if (hasUsedTouchRef.current && !isEnded) {
          ...
          // Draw Pause Button with responsive glassy texture
          ...
          context.fillRect(pauseCircle.x - barGap / 2 - barW, pauseCircle.y - barH / 2, barW, barH);
          context.fillRect(pauseCircle.x + barGap / 2, pauseCircle.y - barH / 2, barW, barH);

          // Restore normal opacity for layout buttons based on customControls
          ...
          for (const key in touchButtonRectsRef.current) {
              ...
              // Cybernetic vector button backplate
              if (isActive) {
                  // Active press style with glowing teal border & fill
                  context.shadowColor = '#5eead4';
                  context.shadowBlur = 12 * scale;
                  context.fillStyle = 'rgba(20, 184, 166, 0.45)'; // Active cyan tinted glass
                  context.strokeStyle = 'rgba(255, 255, 255, 0.95)';
              } else {
                  // Standard modern dark hud circle with soft ambient border
                  context.fillStyle = 'rgba(15, 23, 42, 0.55)'; // Sleek Slate plate
              ...
              context.shadowBlur = 0; // Clear shadow to keep inner details crisp

              // Secondary thin tactile accent ring inside the button
              context.strokeStyle = isActive ? 'rgba(255, 255, 255, 0.45)' : 'rgba(255, 255, 255, 0.1)';
              context.lineWidth = 1 * scale;
              context.beginPath();
              context.arc(x, y, r - 3 * scale, 0, Math.PI * 2);
              context.stroke();

              // --- Draw premium high contrast vector HUD action icons ---
              context.strokeStyle = `rgba(255, 255, 255, ${isActive ? 1.0 : 0.88})`;
              context.fillStyle = `rgba(255, 255, 255, ${isActive ? 1.0 : 0.88})`;
              context.lineWidth = Math.max(2.5, r * 0.08);
              context.lineCap = 'round';
              context.lineJoin = 'round';

              switch (key) {
                  case 'fire':
                  case 'fixedFire': { // High accuracy double-ring crosshair
                      const inner = r * 0.18;
                      const outer = r * 0.48;
                      
                      // Center target dot
                      context.beginPath();
                      context.arc(x, y, r * 0.08, 0, Math.PI * 2);
                      context.fill();
                      
                      // Delicate cross lines
                      context.beginPath();
                      context.moveTo(x, y - inner); context.lineTo(x, y - outer);
                      context.moveTo(x, y + inner); context.lineTo(x, y + outer);
                      context.moveTo(x - inner, y); context.lineTo(x - outer, y);
                      context.moveTo(x + inner, y); context.lineTo(x + outer, y);
                      context.stroke();

                      // Target perimeter circle segment accents
                  ...
                  case 'reload': { // Reload cycle layout (Dynamic curved arrows)
                      // Rounded reload cyclic paths
                      context.save();
                      context.translate(x, y);
                      context.beginPath();
                      context.arc(0, 0, r * 0.38, -Math.PI * 0.4, Math.PI * 0.7);
                      context.stroke();
                      context.beginPath();
                      context.arc(0, 0, r * 0.38, Math.PI * 0.6, Math.PI * 1.7);
                      context.stroke();
                      
                      // Draw sleek arrowhead 1
                      context.rotate(Math.PI * 0.7);
                      context.beginPath();
                      context.moveTo(r * 0.38, -r * 0.1);
                      context.lineTo(r * 0.38 + r * 0.08, r * 0.05);
                      context.lineTo(r * 0.38 - r * 0.08, r * 0.05);
                      context.closePath();
                      context.fill();

                      // Draw sleek arrowhead 2
                      ...
                      // Mini central magazine stack representation
                      context.fillRect(x - r * 0.12, y - r * 0.18, r * 0.24, r * 0.36);
                      break;
                  }
                  case 'heal': { // Perfectly aligned medical cross with circular wrap
                      context.save();
                      context.lineWidth = 1 * scale;
                      context.beginPath();
                      context.arc(x, y, r * 0.52, 0, Math.PI * 2);
                      context.stroke();
                      context.restore();
                      
                      context.fillRect(x - r * 0.1, y - r * 0.35, r * 0.2, r * 0.7);
                      context.fillRect(x - r * 0.35, y - r * 0.1, r * 0.7, r * 0.2);
                      break;
                  }
                  case 'special': { // Ultimate Star Burst graphic (dynamic compass rose)
                      const spikes = 8;
                      const outerRadius = r * 0.52;
                      ...
                      for (let i = 0; i < spikes * 2; i++) {
                          ...
                          context.lineTo(x + r_val * Math.cos(angle), y + r_val * Math.sin(angle));
                          angle += angleStep;
                      }
                      context.closePath();
                      context.fill();
                  ...
                  case 'interact': { // Advanced structural gearwheel cog
                      const teeth = 8;
                      const outerR = r * 0.48;
                      const midR = r * 0.38;
                      const innerR = r * 0.22;
                      ...
                      for (let i = 0; i < teeth * 2; i++) {
                          ...
                          const radius = i % 2 === 0 ? outerR : midR;
                          context.lineTo(x + Math.cos(angle) * radius, y + Math.sin(angle) * radius);
                      }
                      context.closePath();
                      context.fill();

                      // Cut out the center hole cleanly
                      context.globalCompositeOperation = 'destination-out';
                      context.beginPath();
                      context.arc(x, y, holeR, 0, Math.PI * 2);
                      context.fill();
                      context.globalCompositeOperation = 'source-over';
                      
                      // Sleek circular line outline
                      context.lineWidth = 1 * scale;
                      context.beginPath();
                      context.arc(x, y, innerR, 0, Math.PI * 2);
                      context.stroke();
                      break;
                  }
                  case 'switchWeapon': { // Dual-weapon Swap Silhouette (Two staggered side-by-side pistols)
                      context.save();
                      context.translate(x, y);
                      
                      // Upper pistol mockup line
                      context.strokeRect(-r * 0.35, -r * 0.25, r * 0.6, r * 0.14); // Barrel
                      context.fillRect(-r * 0.28, -r * 0.11, r * 0.12, r * 0.22); // Handle
                      
                      // Lower pistol mockup line
                      context.strokeRect(-r * 0.25, r * 0.08, r * 0.6, r * 0.14); // Barrel
                      context.fillRect(-r * 0.18, r * 0.22, r * 0.12, r * 0.22); // Handle
                      
                      // Curving cycle arrows
                      context.lineWidth = 1.5 * scale;
                      context.beginPath();
                      context.arc(0, 0, r * 0.5, -Math.PI * 0.1, Math.PI * 0.4);
                      context.stroke();
                      context.beginPath();
                      context.arc(0, 0, r * 0.5, Math.PI * 0.9, Math.PI * 1.4);
                      context.stroke();
                      context.restore();
                      break;
                  }
                  case 'melee': { // Sleek tactical CQB combat blade/dagger
                      context.save();
                      context.translate(x, y);
                      context.rotate(-Math.PI / 4); // Angle blade diagonally
                      
                      // Crossguard
                      context.fillRect(-r * 0.2, r * 0.16, r * 0.4, r * 0.08);
                      
                      // Grip/hilt
                      context.lineWidth = Math.max(3, r * 0.08);
                      context.beginPath();
                      context.moveTo(0, r * 0.22);
                      context.lineTo(0, r * 0.48);
                      context.stroke();
                      
                      // Blade with sleek shape point
                      context.beginPath();
                      context.moveTo(-r * 0.08, r * 0.14);
                      context.lineTo(-r * 0.08, -r * 0.35);
                      context.lineTo(0, -r * 0.52); // Point
                      context.lineTo(r * 0.08, -r * 0.3);
                      context.lineTo(r * 0.08, r * 0.14);
                      context.closePath();
                      context.fill();
                      context.stroke();
                      context.restore();
                  ...
                  case 'throwableSelect': { // High accuracy tactical fragmentation grenade shape
                      // Grenade body sphere
                      context.beginPath();
                      context.arc(x, y + r * 0.1, r * 0.34, 0, Math.PI * 2);
                      context.fill();
                      context.stroke();
                      
                      // Fuse neck/stem
                      context.fillRect(x - r * 0.11, y - r * 0.38, r * 0.22, r * 0.16);
                      
                      // Safety ring/pin loop
                      context.beginPath();
                      context.arc(x - r * 0.18, y - r * 0.3, r * 0.12, 0, Math.PI * 2);
                      context.stroke();
                      
                      // Grip lever line
                      context.beginPath();
                      context.moveTo(x + r * 0.06, y - r * 0.3);
                      context.quadraticCurveTo(x + r * 0.22, y - r * 0.1, x + r * 0.26, y + r * 0.2);
                      context.stroke();
                      break;
                  }
                  case 'switchThrowable': { // Aesthetic circular reload-style arrowheads representing throwables swap
                      context.save();
                      context.translate(x, y);
                      context.lineWidth = 2 * scale;
                      context.beginPath();
                      context.arc(0, 0, r * 0.42, 0, Math.PI * 1.5);
                      context.stroke();
                      
                      // Draw clean arrow head on tail
                      context.rotate(Math.PI * 1.5);
                      context.beginPath();
                      context.moveTo(r * 0.42, -r * 0.08);
                      context.lineTo(r * 0.42 + r * 0.1, r * 0.1);
                      context.lineTo(r * 0.42 - r * 0.1, r * 0.1);
                      context.closePath();
                      context.fill();
                      context.restore();
                  ...
                  case 'fireModeSwitch': { // High-tech 3-bullet burst indicator icons
                      context.save();
                      context.translate(x, y);
                      // 3 stacked sleek micro cartridge bullets
                      const bulletW = r * 0.08;
                      const bulletH = r * 0.22;
                      const spacing = r * 0.16;
                      
                      const drawBullet = (bx: number, by: number) => {
                          context.fillRect(bx - bulletW/2, by - bulletH/2, bulletW, bulletH);
                          context.beginPath();
                          context.moveTo(bx - bulletW/2, by - bulletH/2);
                          context.lineTo(bx, by - bulletH/2 - bulletW);
                          context.lineTo(bx + bulletW/2, by - bulletH/2);
                          context.closePath();
                          context.fill();
                      };

                      drawBullet(-spacing, -r * 0.04);
                      drawBullet(0, -r * 0.04);
                      drawBullet(spacing, -r * 0.04);
                      
                      // Subtle fire divider line below bullets
                      context.lineWidth = 1 * scale;
                      context.beginPath();
                      context.moveTo(-r * 0.3, r * 0.2);
                      context.lineTo(r * 0.3, r * 0.2);
                      context.stroke();
                      context.restore();
                      break;
                  }
  ...
  useEffect(() => {
    ...
    // Prevent default touch gestures (pinch-to-zoom, swipe-to-scroll, bounce, pull-to-refresh)
    // ONLY on the game canvas itself to preserve interactive menus and options outside!
    ...
    // Prevent browser zoom wheel gestures (especially with Ctrl/Pinch on trackpad)
    ...
    // Prevent iOS/Safari desktop gestures zooming
    const preventGesture = (e: Event) => {
      e.preventDefault();
    };

    // Prevent context menu (long press gesture or right click)
    const preventCtxMenu = (e: Event) => {
      e.preventDefault();
    };
    ...
    window.addEventListener('contextmenu', preventCtxMenu, { passive: false });
    ...
    return () => {
      ...
      window.removeEventListener('contextmenu', preventCtxMenu);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const handlePlayerJoined = (payload: PlayerState) => {
        ...
        const existingIdx = remotePlayersRef.current.findIndex(p => p.id === payload.id);
        const newPlayer: RemotePlayer = {
            ...payload,
            targetX: payload.x,
            targetY: payload.y,
            lastUpdateTime: performance.now(),
            isShooting: !!payload.isShooting,
            playerId: payload.id, // For drawing username tags compat
            lastUpdate: Date.now() // For status window filtering compat
        };
        
        if (existingIdx >= 0) {
            remotePlayersRef.current[existingIdx] = newPlayer;
        } else {
            remotePlayersRef.current.push(newPlayer);
        }
    ...
    const handlePlayerLeft = (payload: { id: string }) => {
        try { if (networkClient && payload.id === networkClient.ownId) return; } catch (e) {}
        const updated = remotePlayersRef.current.filter(p => p.id !== payload.id);
        remotePlayersRef.current = updated;
        setRemotePlayers(updated);
    };
    const handlePlayerUpdate = (payload: PlayerState & { isShooting: boolean }) => {
        try { if (networkClient && payload.id === networkClient.ownId) return; } catch (e) {}
        const p = remotePlayersRef.current.find(p => p.id === payload.id);
        if (p) {
    ...
    const handleFireWeapon = (payload: FireEventPayload) => {
        ...
        const p = remotePlayersRef.current.find(p => p.id === payload.ownerId);
    ...
    const handlePlayerAction = (payload: any) => {
        try {
            const { action, payload: actionPayload, playerId } = payload;
            if (action === 'player-reset') {
                const p = remotePlayersRef.current.find(p => p.id === playerId);
                if (p) {
                    p.health = 100;
                }
            } else if (action === 'flashbang') {
                const ownId = networkClient?.ownId;
                if (actionPayload?.targetId === ownId) {
                    // Apply flashbang effect to local player
                    const maxFlashDuration = 2.5;
                    const minFlashDuration = 0.5;
                    // Directional reduction: if facing away, shorter flash
                    const dx = actionPayload.x - playerRef.current.x;
                    const dy = actionPayload.y - playerRef.current.y;
                    const angleToFlash = Math.atan2(dy, dx);
                    const angleDiff = Math.abs(normalizeAngle(angleToFlash - playerDirectionRef.current));
                    const flashFactor = Math.max(0, 1 - (angleDiff / Math.PI));
                    const flashDuration = minFlashDuration + (maxFlashDuration - minFlashDuration) * flashFactor;
                    playerRef.current.flashTimer = Math.max(playerRef.current.flashTimer, flashDuration);
                }
                ...
                const attackerName = payload.attackerName || remotePlayersRef.current.find(p => p.id === attackerId)?.name || attackerId || 'UNKNOWN';
                const victimName = payload.victimName || remotePlayersRef.current.find(p => p.id === victimId)?.name || victimId || 'UNKNOWN';
    ...
    const handleScoreUpdate = (payload: { players: Array<{ id: string; name?: string; team?: 'red' | 'blue'; kills: number; deaths: number }>; totalKills: number; mode: 'tdm' | 'ffa' | '1v1'; teamScores?: { red: number; blue: number } }) => {
        try {
            const ownId = networkClient?.ownId;
            const previous = roomScoresRef.current;
            const next: Record<string, { kills: number; deaths: number; name?: string; team?: 'red' | 'blue' }> = {};
            payload.players.forEach(p => {
                next[p.id] = { kills: p.kills, deaths: p.deaths, name: p.name, team: p.team };
            });
            roomScoresRef.current = next;
            setRoomScores(next);
            setTotalRoomKills(payload.totalKills);
            if (payload.teamScores) {
                teamScoresRef.current = payload.teamScores;
    ...
    const handleKillFeed = (payload: { victimId: string; victimName?: string; attackerId: string; attackerName?: string; timestamp: number }) => {
        try {
            const ownId = networkClient?.ownId;
            if (payload.attackerId === ownId) return; // already handled by score-update reward
            const attackerName = payload.attackerName || remotePlayersRef.current.find(p => p.id === payload.attackerId)?.name || payload.attackerId || 'UNKNOWN';
            const victimName = payload.victimName || remotePlayersRef.current.find(p => p.id === payload.victimId)?.name || payload.victimId || 'UNKNOWN';
            setPurchaseMessage(`${attackerName} ELIMINATED ${victimName}`);
            setTimeout(() => setPurchaseMessage(null), 3000);
        } catch (e) {}
    };
    ...
    const handleMatchTimer = (payload: { remainingMs: number; totalMs: number }) => {
        try {
            setMatchRemainingMs(payload.remainingMs);
        } catch (e) {}
    };

    const handleMatchEnded = (payload: { mode: 'tdm' | 'ffa' | '1v1'; winner: 'red' | 'blue' | 'draw' | null; teamScores?: { red: number; blue: number }; players: Array<{ id: string; name: string; team?: string; kills: number; deaths: number }> }) => {
        try {
    ...
    networkClient.on('match-timer' as any, handleMatchTimer as any);
    networkClient.on('match-ended' as any, handleMatchEnded as any);

    const handleTeamSelectionFailed = (payload: { team: 'red' | 'blue'; reason?: string }) => {
        try {
            setTeamSelectError(payload.reason || 'Team full');
        } catch (e) {}
    };

    const handleSelectTeamResponse = (payload: { team: 'red' | 'blue' }) => {
        try {
            setTeamSelectError(null);
        } catch (e) {}
    };

    networkClient.on('team-selection-failed' as any, handleTeamSelectionFailed as any);
    networkClient.on('select-team' as any, handleSelectTeamResponse as any);
    ...
    const handlePlayerHit = (payload: { targetId: string; damage: number; attackerId: string; impact?: { x: number; y: number }; sourceDir?: { x: number; y: number } }) => {
        try {
            ...
            if (payload.targetId === own) {
                ...
                applyDamageToPlayer(payload.damage, payload.sourceDir || { x: 0, y: 0 }, payload.impact || { x: playerRef.current.x, y: playerRef.current.y }, payload.attackerId);
            }
        } catch (e) {}
    };
    ...
    return () => {
        ...
        networkClient.off('team-selection-failed' as any, handleTeamSelectionFailed as any);
        networkClient.off('select-team' as any, handleSelectTeamResponse as any);
        networkClient.off('match-timer' as any, handleMatchTimer as any);
        networkClient.off('match-ended' as any, handleMatchEnded as any);
  ...
  useEffect(() => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'e':
            ...
            if (takedownHintEnemyRef.current) {
                ...
                if (doorToInteract && !doorToInteract.locked && !playerRef.current.isHealing) {
                    const isDoubleClick = (now - lastEKeyPressTimeRef.current < 300) && (lastInteractedDoorIdRef.current === doorToInteract.id);
                    ...
                    if (isDoubleClick && dotProd < 0) {
                        doorToInteract.targetAngle = doorToInteract.closedAngle;
        ...
        case 'q': 
            ...
            if (player.currentWeaponIndex === 2) { player.currentWeaponIndex = previousWeaponIndexRef.current; }
            else { previousWeaponIndexRef.current = player.currentWeaponIndex; player.currentWeaponIndex = 2; }
            break;
    ...
    <div className="relative w-full h-full font-mono">
        ...
        {isMultiplayer && gameRoom && (
            <div className="absolute top-4 right-4 z-40 bg-black/60 px-4 py-2 rounded-md border border-teal-500/50 text-teal-300 font-mono">
                ...
                {typeof matchRemainingMs === 'number' && matchRemainingMs >= 0 && (
                    <div className="text-xs mt-0.5 text-amber-300 font-bold">
                        {language === 'zh' ? '剩余时间' : 'TIME'}: {formatMatchTime(matchRemainingMs)}
                    </div>
                )}
        ...
        {isMultiplayer && matchEnded && (
            <div className="absolute inset-0 z-[60] bg-black/90 flex items-center justify-center p-4">
                <div className="bg-gray-950 border-2 border-teal-500 rounded-lg p-8 w-full max-w-md shadow-lg shadow-teal-500/20 text-white text-center">
                    <h2 className="text-3xl font-bold tracking-widest text-teal-300 mb-2">{language === 'zh' ? '对局结束' : 'MATCH ENDED'}</h2>
                    {matchEnded.mode === 'tdm' && matchEnded.teamScores && (
                        <div className="flex items-center justify-center gap-4 mb-4">
                            <span className="text-red-400 font-bold text-xl">RED {matchEnded.teamScores.red}</span>
                            <span className="text-gray-500 font-bold text-sm">VS</span>
                            <span className="text-blue-400 font-bold text-xl">BLUE {matchEnded.teamScores.blue}</span>
                        </div>
                    )}
                    {matchEnded.winner === 'draw' ? (
                        <div className="text-2xl font-bold text-gray-300 mb-6">{language === 'zh' ? '平局' : 'DRAW'}</div>
                    ) : (
                        (() => {
                            const isWinner = matchEnded.mode === 'tdm'
                                ? matchEnded.winner === playerTeamRef.current
                                : (matchEnded.players[0]?.id === networkClient?.ownId);
                            return (
                                <div className={`text-2xl font-bold mb-6 ${isWinner ? 'text-green-400' : 'text-red-400'}`}>
                                    {isWinner ? (language === 'zh' ? '胜利' : 'VICTORY') : (language === 'zh' ? '失败' : 'DEFEAT')}
                                </div>
                            );
                        })()
                    )}
                    <div className="space-y-2 mb-6">
                        {matchEnded.players
                            ...
                            .slice(0, 8)
                            .map((p, idx) => (
                                <div key={p.id} className="grid grid-cols-12 gap-2 items-center px-3 py-2 rounded border border-gray-800 bg-gray-900/30">
                                    <div className="col-span-1 text-center font-bold text-gray-400">{idx + 1}</div>
                                    <div className="col-span-5 font-mono text-sm truncate text-left">{p.name}</div>
                                    <div className="col-span-3 text-center font-bold">{p.kills} / {p.deaths}</div>
                                    <div className="col-span-3 text-center font-bold text-teal-300">{p.deaths === 0 ? p.kills : (p.kills / p.deaths).toFixed(2)}</div>
                                </div>
                            ))}
                    </div>
                    <button
                    ...
                    >
                        {language === 'zh' ? '返回大厅' : 'RETURN TO LOBBY'}
                    </button>
                </div>
            </div>
        )}
                    ...
                    <div className="flex-grow overflow-y-auto">
                        {scoreboardTab === 'room' ? (
                            <div className="space-y-2">
                                {gameRoom.mode === 'tdm' && (
                                    <div className="mb-3 p-3 rounded border border-gray-800 bg-gray-900/50 flex items-center justify-center gap-4">
                                        <span className="text-red-400 font-bold text-lg">RED {teamScores.red}</span>
                                        <span className="text-gray-500 font-bold text-sm">VS</span>
                                        <span className="text-blue-400 font-bold text-lg">BLUE {teamScores.blue}</span>
                                    </div>
                                )}
                                ...
                                {Object.entries(roomScores)
                                    ...
                                    .map(([id, stats]) => {
                                        ...
                                        const s = stats as { kills: number; deaths: number; name?: string; team?: 'red' | 'blue' };
        ...
        {isMultiplayer && gameRoom?.mode === 'tdm' && showTeamSelect && (
            <div className="absolute inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
                <div className="bg-gray-950 border-2 border-teal-500 rounded-lg p-6 w-full max-w-md shadow-lg shadow-teal-500/20 text-white">
                    <h2 className="text-2xl font-bold tracking-widest text-teal-300 mb-2 text-center">SELECT TEAM</h2>
                    <p className="text-center text-gray-400 text-sm mb-6">{language === 'zh' ? '选择你的队伍' : 'Choose your side'}</p>
                    {(() => {
                        const players = gameRoom?.players || {};
                        const redCount = Object.values(players).filter((p: any) => p.team === 'red').length;
                        const blueCount = Object.values(players).filter((p: any) => p.team === 'blue').length;
                        const maxPerTeam = Math.max(1, Math.floor((gameRoom?.maxPlayers || 8) / 2));
                        const redFull = redCount >= maxPerTeam;
                        const blueFull = blueCount >= maxPerTeam;
                        const selectTeam = (team: 'red' | 'blue') => {
                            setTeamSelectError(null);
                            if (networkClient) {
                                (networkClient as any).send('select-team', { team });
                            }
                        };
                        return (
                            <div className="flex flex-col gap-4">
                                <button
                                    ...
                                    disabled={redFull}
                                    className={`w-full py-4 rounded-md font-bold text-lg tracking-widest border-2 transition-colors duration-200 ${
                                        redFull
                    ...
                    {teamSelectError && (
                        <div className="mt-4 text-center text-red-400 text-sm font-bold">{teamSelectError}</div>
                    )}
                </div>
            </div>
        )}
